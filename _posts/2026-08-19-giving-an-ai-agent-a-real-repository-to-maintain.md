...
and Deploy refuses a tag that doesn't match.
...
I migrated the project to the current BuildKit-based actions and native builds for both supported architectures:

- amd64
- aarch64

The release then publishes one multi-architecture GHCR image.

Home Assistant can reference the generic image name and the registry resolves the correct architecture.
...
Fixable HIGH/CRITICAL OS findings block.

Inherited findings stay visible so I know they exist, but I don't pretend this repository can rebuild an upstream binary it doesn't own.
...
The image could still build, so it was easy to miss.

Once I added a strict release-tag comparison, it surfaced.
...
The repo-specific skill contains procedure.
...
Long-term memory is useful for things like:

- which repo I maintain
- where the Azure checkout lives
- how I prefer to be notified
- whether I want release approval before shipping

It is a bad source of truth for:

- current RustDesk version
- latest release
- current `main` SHA
- open PRs
- CI status
...
For me that separation looks like:
...
If the shipped container changes, users need a packaging revision before they receive it.
...
I don't want it autonomously:

- pushing directly to `main`
- bypassing CI
- weakening a security gate
- merging a release
- publishing a release
- changing encryption/network defaults
- posting a sensitive maintainer response
...
Right now the skill says:

> don't direct-push to `main`

I also want GitHub to say:

> you can't direct-push to `main`
...
It turned into a useful little example of:
